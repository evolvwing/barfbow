...
def r_palette_script(rows: list[tuple[str, float, float, float]], palette_name: str) -> str:
    """Return a self-documenting R palette object with ggplot2 helpers."""
    colors = ", ".join(f'"{row[0]}"' for row in rows)
    return "\n".join(
        [
            "# Generated by barfbow.",
            "# Quick start:",
            f'#   source("{palette_name}.R")',
            f"#   plot(1:length({palette_name}), col = {palette_name}, pch = 19)",
            "#   ggplot(...) + scale_colour_barfbow()",
            "#   ggplot(...) + scale_fill_barfbow()",
            "# For continuous ggplot2 data, add: discrete = FALSE",
            "",
            "# Palette data: a compact character vector of hexadecimal sRGB colors.",
            f"{palette_name} <- c({colors})",
            "",
            "# Stable aliases: these let the helper functions work regardless of the",
            "# generated palette name and retain that name as metadata.",
            f"barfbow_palette <- {palette_name}",
            f'barfbow_palette_name <- "{palette_name}"',
            "",
            "# Return the palette colors. Set reverse = TRUE to invert their order.",
            "barfbow_colors <- function(reverse = FALSE) {",
            "  colors <- unname(barfbow_palette)",
            "  if (reverse) rev(colors) else colors",
            "}",
            "",
            "# Apply the palette to ggplot2's colour aesthetic.",
            "# discrete = TRUE maps categories; FALSE creates a continuous gradient.",
            "scale_colour_barfbow <- function(..., reverse = FALSE, discrete = TRUE) {",
            "  colors <- barfbow_colors(reverse)",
            "  if (discrete) {",
            ...
            "  }",
            "}",
            "# American-English alias for scale_colour_barfbow().",
            "scale_color_barfbow <- scale_colour_barfbow",
            "",
            "# Apply the palette to ggplot2's fill aesthetic.",
            "# discrete = TRUE maps categories; FALSE creates a continuous gradient.",
            "scale_fill_barfbow <- function(..., reverse = FALSE, discrete = TRUE) {",
            "  colors <- barfbow_colors(reverse)",
            "  if (discrete) {",
